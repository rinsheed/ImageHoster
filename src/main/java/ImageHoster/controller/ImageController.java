...
public class ImageController {
    ...
    public String showImage(@PathVariable("imageId") Integer imageId, Model model, HttpSession session) {
        Image image = imageService.getImage(imageId);

        //If an unauthorised user tries to edit the image, the error message will be displayed.
        if(session.getAttribute("invalidEditTrial") == "true") {
            model.addAttribute("editError", "true");
        }
        else {
            model.addAttribute("editError", "false");
        }
        session.setAttribute("invalidEditTrial", "false");

        //If an unauthorised user tries to delete the image, the error message will be displayed.
        if(session.getAttribute("invalidDeleteTrial") == "true") {
            model.addAttribute("deleteError", "true");
        }
        else {
            model.addAttribute("deleteError", "false");
        }
        session.setAttribute("invalidDeleteTrial", "false");
    ...
    public String editImage(@RequestParam("imageId") Integer imageId, Model model, HttpSession session) {
        ...
        session.setAttribute("invalidEditTrial", "true");
    ...
    public String deleteImageSubmit(@RequestParam(name = "imageId") Integer imageId, HttpSession session) {
        ...
        session.setAttribute("invalidDeleteTrial", "true");
        return "redirect:/images/" + imageId + "/" + image.getTitle();
    }

    //This controller method is called when the request pattern is of type '/image/{imageId}/{title}/comments' and also the incoming request is of POST type
    //The method receives comment text & the corresponding imageId to which it needs be stored in the database, and now the comment will be sent to the business logic to be persisted in the database
    //After you get the comment text, set the user of the comment by getting the logged in user from the Http Session
    //Set the date on which the comment is posted
    //After storing the comment, this method directs to the image view page, which display all the comments & edit & delete option.
    @RequestMapping(value = "/image/{imageId}/{title}/comments", method = RequestMethod.POST)
    public String postComment(@RequestParam(name = "comment") String text, @PathVariable(name = "imageId") Integer imageId, HttpSession session){
